...
class FeatureInteractions:
    ...
    def GetFeatureInteractionsWithLeafStatistics(self):
        return sorted(
            [self.interactions[key] for key in self.interactions.keys() if self.interactions[key].HasLeafStatistics],
            key=_comparer)
    ...
    def Merge(self, other):
        for key in other.interactions.keys():
            ...
            if not (key in self.interactions):
                ...
                self.interactions[key].AverageFScoreWeighted = self.interactions[key].FScoreWeighted / \
                                                               self.interactions[key].FScore
                ...
                self.interactions[key].AverageTreeIndex = self.interactions[key].TreeIndex / self.interactions[
                    key].FScore
                self.interactions[key].TreeDepth += fi.TreeDepth
                self.interactions[key].AverageTreeDepth = self.interactions[key].TreeDepth / self.interactions[
                    key].FScore
...
class XgbModel:
    ...
    def GetFeatureInteractions(self, maxInteractionDepth, maxDeepening):
        ...
        for i, tree in enumerate(self.XgbTrees):
            ...
            treeNodes = []
            self.CollectFeatureInteractions(tree, treeNodes, currentGain=0.0, currentCover=0.0, pathProbability=1.0,
                                            depth=0, deepening=0)
    ...
    def CollectFeatureInteractions(self, tree, currentInteraction, currentGain, currentCover, pathProbability, depth,
                                   deepening):
        if tree.node.IsLeaf:
            return
        ...
        fi = FeatureInteraction(currentInteraction, currentGain, currentCover, pathProbability, depth, self._treeIndex,
                                1)
        ...
        if (depth < self._maxDeepening) or (self._maxDeepening < 0):
            ...
            self.CollectFeatureInteractions(tree.left, newInteractionLeft, 0.0, 0.0, pathProbabilityLeft, depth + 1,
                                            deepening + 1)
            self.CollectFeatureInteractions(tree.right, newInteractionRight, 0.0, 0.0, pathProbabilityRight, depth + 1,
                                            deepening + 1)
        ...
        self.CollectFeatureInteractions(tree.left, currentInteractionLeft, currentGain, currentCover,
                                        pathProbabilityLeft, depth + 1, deepening)
        self.CollectFeatureInteractions(tree.right, currentInteractionRight, currentGain, currentCover,
                                        pathProbabilityRight, depth + 1, deepening)
...
class XgbModelParser:
    def __init__(self, verbosity=0):
        self._verbosity = verbosity
        self.nodeRegex = re.compile(
            r'(\d+):\[(.*?)(?:<(.+)|)\]\syes=(.*),no=(.*?),(?:missing=.*,)?gain=(.*),cover=(.*)')
...
def GetStatistics(booster, dump, feature_names=None, MaxTrees=100, MaxInteractionDepth=2, MaxDeepening=-1,
                  SortBy='Gain'):
...
def WriteFsTree(root, index):
    ncur = root
    with open('fstree.txt', 'a') as f:
        f.write(str(index) + " ")
        f.write(" IsLeaf: " + str(ncur.node.IsLeaf))
        f.write(" Feature: " + str(ncur.node.Feature))
        f.write(" Cover: " + str(ncur.node.Cover))
        f.write(" Gain: " + str(ncur.node.Gain))
        f.write("\n")
        if ncur.left is not None:
            WriteFsTree(ncur.left, 2 * index + 1)
        if ncur.right is not None:
            WriteFsTree(ncur.right, 2 * index + 2)
